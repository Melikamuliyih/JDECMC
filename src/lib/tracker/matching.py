...
def elddistancematric(bboxes1, bboxes2):
    
    elgiou = torch.zeros((len(bboxes1), len(bboxes2)), dtype=float)
    if len(bboxes1) * len(bboxes2) == 0:
        return elgiou
    ...
    rows = bboxes1.shape[0]
    cols = bboxes2.shape[0]
    elgiou = torch.zeros((rows, cols))
    exchange = False
    if bboxes1.shape[0] > bboxes2.shape[0]:
        bboxes1, bboxes2 = bboxes2, bboxes1
        elgiou = torch.zeros((cols, rows))
        exchange = True
    ...
    elgiou = iou - (outer_area - union)/outer_area
    elgiou = torch.clamp(elgiou,min=-1.0,max = 1.0)
    if exchange:
        elgiou = elgiou.T
    return elgiou
...
def location_distance(atracks, btracks, type="eldistance"):
    """
    Compute cost based on IoU
    :type atracks: list[STrack]
    :type btracks: list[STrack]
    ...
    if type=="eldistance":
        _ious = elddistancematric(atlbrs, btlbrs)
    else:
        _ious = ious(atlbrs, btlbrs)
    cost_matrix = 1 - _ious
